...
def refine_path(path_nodes, epsilon=50):
    ...
    for i in range(len(path_nodes)-1):
        ...
        if dist <= epsilon:
            ...
            # number of extra segments to add
            steps = math.ceil(dist / epsilon)
            for k in range(1, steps+1):
                lat = p1["lat"] + (p2["lat"] - p1["lat"]) * k/steps
                lon = p1["lon"] + (p2["lon"] - p1["lon"]) * k/steps
                refined.append({
                    "node": None,  # interpolated point, not a real node
                    "lat": lat,
                    "lon": lon,
                    "distance_from_start_m": p1["distance_from_start_m"] + dist * k/steps
                })
    return refined
