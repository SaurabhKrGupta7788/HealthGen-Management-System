...
def refine_path(path_nodes, epsilon=50):
    """
    Refine path so that distance between consecutive nodes is exactly epsilon (50m),
    and the final leftover is ≤ epsilon.
    """
    refined = []
    for i in range(len(path_nodes)-1):
        ...
        if dist <= epsilon:
            ...
            # direction vector in lat/lon
            dlat = p2["lat"] - p1["lat"]
            dlon = p2["lon"] - p1["lon"]

            # how many full 50m steps fit in
            full_steps = int(dist // epsilon)
            step_fraction = epsilon / dist  # fraction along the line per step
            ...
            for k in range(1, full_steps+1):
                ...
                refined.append({
                    ...
                    "lat": lat,
                    "lon": lon,
                    "distance_from_start_m": p1["distance_from_start_m"] + k * epsilon
                })

            # finally, add the real node (whatever leftover distance, ≤ epsilon)
            refined.append({
                "node": p2["node"],
                "lat": p2["lat"],
                "lon": p2["lon"],
                "distance_from_start_m": p1["distance_from_start_m"] + dist
            })

    return refined

# ------------------------------
# integrate into previous result
# ------------------------------
...
def all_routes_graphml(coord1, coord2, max_paths=3, epsilon=50):
    ...
    # Convert MultiDiGraph -> DiGraph (keep shortest edges, attributes, CRS)
    G = nx.DiGraph()
